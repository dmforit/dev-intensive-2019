...
//        benderImage = findViewById(R.id.iv_bender) as ImageView
     ...
     *
     * используется для специальных действий, коотрые должны выполняться при повторном запуске Activity.
     */
    ...
    /**
     * При вызове onStart() окно ещё не видно пользователю, но скоро будет видно.
     * Вызывается непосредственно перед тем, как активность становится видимой пользователю.
     *
     * Чтение из баз данных
    ...
    /**
     * Вызывается, когда Activity начнёт взаимодействовать с пользователем.
     *
    ...
    /**
     * Метод onStop() вызывается, когда Activity становится невидимым для пользователся.
     ...
     * которые нужны исключительно для обновления пользовательского интерфейса
     *
     * Не вызывается при вызове finish() у Activity
     */
    override fun onStop() {
        super.onStop()
